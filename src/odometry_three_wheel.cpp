...
OdometryThreeWheel::OdometryThreeWheel(int8_t leftEncoderPort, int8_t rightEncoderPort, int8_t centerEncoderPort, double sL, double sR, double sS)
    : Odometry(), sL(sL), sR(sR), sS(sS), leftEncoder(leftEncoderPort), rightEncoder(rightEncoderPort), centerEncoder(centerEncoderPort) {
...
void OdometryThreeWheel::updatePose() {
    ...
    double dC = (((double) centerEncoderNewPos - (double) centerEncoderLastPos) / 36000.0) * 6.28318;
    ...
    if (doubleEquals(deltaThetaRad, 0.0, 0.01)) {
        ...
        localOffsetX = (2.0 * sin(deltaThetaRad / 2.0) * (dC / deltaThetaRad) + sS);
