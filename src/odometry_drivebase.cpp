...
void DrivebaseOdometry::updatePose() {
    ...
    uint32_t leftMotorGroupNewPos = leftMotorGroup.get_raw_position(0);
    uint32_t rightMotorGroupNewPos = rightMotorGroup.get_raw_position(0);
    double dL = (((double) leftMotorGroupNewPos - (double) leftMotorGroupLastPos) / 360.0) * (57.0 / 39.0) * 10.205;
    double dR = (((double) rightMotorGroupNewPos - (double) rightMotorGroupLastPos) / 360.0) * (57.0 / 39.0) * 10.205;
    double d = (dL + dR) / 2.0;
